...
async def send_reminder(context: ContextTypes.DEFAULT_TYPE, event, chat_id: int):
...
async def send_reminder_improved(context: ContextTypes.DEFAULT_TYPE, event, chat_id: int):
    ...
    reminder_text = f"⏰ Напоминание о мероприятии:\n\n"
    reminder_text += f"📋 {event.protocol}\n"
    if event.description:
        # Ограничиваем длину описания
        desc = event.description[:100] + "..." if len(event.description) > 100 else event.description
        reminder_text += f"📝 {desc}\n"
    reminder_text += f"👤 {event.responsible_person}\n"
    reminder_text += f"📅 {event.deadline.strftime('%d.%m.%Y %H:%M')}\n"
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if days_until_deadline < 0:
                        ...
                        overdue_text += f"📋 {event.protocol}\n"
                        if event.description:
                            # Ограничиваем длину описания
                            desc = event.description[:100] + "..." if len(
                                event.description) > 100 else event.description
                            overdue_text += f"📝 {desc}\n"
                        overdue_text += f"👤 {event.responsible_person}\n"
                        overdue_text += f"📅 {event.deadline.strftime('%d.%m.%Y %H:%M')}\n"
...
async def schedule_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает текущее состояние планировщика"""
    from .models import BotSettings
    from asgiref.sync import sync_to_async
    ...
    try:
        ...
        settings_obj = await sync_to_async(BotSettings.objects.get)(id=1)
        db_time = settings_obj.daily_check_time

        status_text = f"📊 **Статус планировщика:**\n"
        status_text += f"• Время в БД: {db_time.hour:02d}:{db_time.minute:02d}\n"
        status_text += f"• Интервал напоминаний: {settings_obj.reminder_interval} мин\n"

        if scheduler:
            job = scheduler.get_job('daily_reminders')
            if job:
                # Пытаемся получить информацию о триггере
                try:
                    trigger = job.trigger
                    current_hour = trigger.fields[4].__str__()  # hour field
                    current_minute = trigger.fields[5].__str__()  # minute field

                    # Конвертируем в читаемый формат
                    if current_hour == '*':
                        current_hour_str = "каждый час"
                    else:
                        current_hour_str = f"{int(current_hour):02d}"

                    if current_minute == '*':
                        current_minute_str = "каждую минуту"
                    else:
                        current_minute_str = f"{int(current_minute):02d}"

                    status_text += f"• Текущее расписание: {current_hour_str}:{current_minute_str}\n"

                    # Проверяем соответствие
                    if current_hour != '*' and current_minute != '*':
                        if int(current_hour) == db_time.hour and int(current_minute) == db_time.minute:
                            status_text += "• ✅ **Синхронизировано**\n"
                        else:
                            status_text += "• ❌ **НЕ синхронизировано!**\n"

                except Exception as trigger_error:
                    status_text += f"• ⚠️ Ошибка чтения триггера: {trigger_error}\n"

                if job.next_run_time:
                    moscow_tz = pytz.timezone('Europe/Moscow')
                    next_run = job.next_run_time.astimezone(moscow_tz)
                    status_text += f"• Следующий запуск: {next_run.strftime('%d.%m.%Y %H:%M')}\n"
                else:
                    status_text += "• Следующий запуск: не запланирован\n"

                # Показываем активные задачи
                jobs = scheduler.get_jobs()
                status_text += f"• Активных задач: {len(jobs)}\n"
            else:
                status_text += "• ❌ Задача не найдена\n"
        else:
            status_text += "• ❌ Планировщик не запущен\n"
...
async def force_update_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        settings_obj = await sync_to_async(BotSettings.objects.get)(id=1)
        new_time = settings_obj.daily_check_time
        ...
        if scheduler:
            ...
            # Получаем обновленную задачу для показа следующего запуска
            job = scheduler.get_job('daily_reminders')
            next_run_info = ""
            if job and job.next_run_time:
                moscow_tz = pytz.timezone('Europe/Moscow')
                next_run = job.next_run_time.astimezone(moscow_tz)
                next_run_info = f"\n⏰ Следующий запуск: {next_run.strftime('%d.%m.%Y %H:%M')}"

            await update.message.reply_text(
                f"✅ Расписание принудительно обновлено!\n"
                f"Новое время: {new_time.hour:02d}:{new_time.minute:02d}"
                f"{next_run_info}"
            )
            print(f"🔄 Принудительное обновление: {new_time.hour:02d}:{new_time.minute:02d}")
        else:
...
def setup_bot() -> Application:
    ...
    application.add_handler(CommandHandler("status", schedule_status))
    application.add_handler(CommandHandler("force_update", force_update_schedule))
